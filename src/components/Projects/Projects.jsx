    ...
    <>
      ...
      <section className="projects" id="projects">
        <header className="categories">
          ...
          >
            {t("projectDescription.all")}
          </StyledButton>
          <StyledButton
            categorie={categorie}
            item={"React"}
            onClick={() => setCategories("React")}
          >
            React
          </StyledButton>
          <StyledButton
          ...
          <StyledButton
            ...
            item={"Games"}
            className="opacity"
            onClick={() => setCategories("Games")}
          >
            {t("projectDescription.game")}
          </StyledButton>
        </header>
        <article className="container">
          <div className="description-categorie">
            {categorie === "" ? (
              t("projectDescription.allproject")
            ) : categorie ? (
              <div>
                {t("projectDescription.category")} <b>{categorie}</b>
              </div>
            ) : (
              "Error"
            )}
                    ...
                    <div className="project-info">
                      <div className="slide">
                        ...
                        <div className="info-button">
                          <NavLink to={`/project/${item.route}`}>
                            {" "}
                            <button className="button-left" type="button">
                              {t("projectDescription.details")}
                            </button>
                          </NavLink>
                          <a href={item.live}>
                            {" "}
                            <button className="button-right" type="button">
                              {t("projectDescription.live")}
                            </button>
                          </a>
                        </div>
                      </div>
...
const StyledButton = styled.button`
  background-color: ${({ categorie, item }) =>
    categorie === item ? "white" : "#AFD5ED"};
